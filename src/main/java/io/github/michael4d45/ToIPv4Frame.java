...
public class ToIPv4Frame extends Frame {
  ...
  public int[] buildSymbols() {
    ...
    symbols[4] = (srcPort >> 4) & 0xF; // SRC_PORT_HI
    symbols[5] = srcPort & 0xF; // SRC_PORT_LO
    // DST_IP: 8 nibbles
    for (int i = 0; i < 4; i++) {
      symbols[6 + 2 * i] = (dstIp[i] >> 4) & 0xF;
      symbols[7 + 2 * i] = dstIp[i] & 0xF;
    }
    symbols[14] = (dstPort >> 4) & 0xF; // DST_PORT_HI
    symbols[15] = dstPort & 0xF; // DST_PORT_LO
